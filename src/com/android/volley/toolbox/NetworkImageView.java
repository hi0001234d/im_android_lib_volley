...
public class NetworkImageView extends ImageView
{
	...
	/** im_: Allow lazy loading **/
	// if allowed in that case network image view first display small version
	// than larger one
	public boolean is_allow_lazy_loading = false;
	public ProgressBar lazy_loader = null; 
	...
	/**
	 * im_ extended function for lazy loading
	 */
	public void setImageUrlLazy( String url, ImageLoader imageLoader )
	{
		mUrl = url;
		mImageLoader = imageLoader;
		// The URL has potentially changed. See if we need to load it.
		loadImageIfNecessaryLazy(false);
	}

	/**
	 * im_ extended function for lazy loading
	 */
	void loadImageIfNecessaryLazy( final boolean isInLayoutPass )
	{
		int width = getWidth();
		...
		if ( mImageContainer != null && mImageContainer.getRequestUrl() != null )
		{
			...
			{
				// allow_lazy_loading
				if ( !is_allow_lazy_loading )
				{
					// Log.v("lazy", "not detected");
		...
		// Load the new image
		// from the network.
		ImageContainer newContainer = mImageLoader.getLazy(mUrl, new ImageListener()
		{
			@Override
		...
		}, maxWidth, maxHeight, lazy_loader);
